        ...
        <div class="m-0">
            ...
            <div class="row g-5 mb-12">
                <div class="col-sm-6">
                    ...
                    <div
                        class="fw-bold fs-7 text-gray-600">@if(isset($invoice->patient->address) && !empty($invoice->patient->address))
                ...
                <div class="col-sm-6">
                    ...
                    <div
                        class="fw-bold fs-7 text-gray-600">{{ ($hospitalAddress=="") ? __('messages.common.n/a') : $hospitalAddress }}</div>
                </div>
            </div>
            <div class="row g-5 mb-12">
                <div class="col-sm-6">
                    ...
                    <table class="table mb-3">
                        ...
                        @foreach($invoice->invoiceItems as $index => $invoiceItem)
                            <tr class="fw-bolder text-gray-700 fs-5 text-end">
                                ...
                                <td class="pt-6">
                                    <b>{{ getCurrencySymbol() }}</b> {{ number_format($invoiceItem->price) }}</td>
                ...
                <div class="d-flex justify-content-end">
                    <div class="mw-300px">
                        <div class="d-flex flex-stack mb-3">
                            <div
                                class="fw-bold pe-10 text-gray-600 fs-7">{{ __('messages.invoice.sub_total').(':') }}</div>
                            <div
                                class="text-end fw-bolder fs-6 text-gray-800">{{ getCurrencySymbol() }} {{ number_format($invoice->amount,2) }}</div>
                        </div>
                        <div class="d-flex flex-stack mb-3">
                            <div
                                class="fw-bold pe-10 text-gray-600 fs-7">{{ __('messages.invoice.discount').(':') }}</div>
                            <div
                                class="text-end fw-bolder fs-6 text-gray-800">{{ getCurrencySymbol() }} {{ number_format(($invoice->amount * $invoice->discount / 100),2) }}</div>
                        </div>
                        <div class="d-flex flex-stack">
                            <div class="fw-bold pe-10 text-gray-600 fs-7">{{ __('messages.invoice.total').(':') }}</div>
                            <div
                                class="text-end fw-bolder fs-6 text-gray-800">{{ getCurrencySymbol() }} {{ number_format($invoice->amount - ($invoice->amount * $invoice->discount / 100),2)}}</div>
                        </div>
                    </div>
                </div>
            </div>
...
<div class="m-0">
    <div class="d-print-none border border-dashed border-gray-300 card-rounded h-lg-100 min-w-md-350px p-9 bg-lighten">
        ...
        <div class="mb-6">
            ...
            <div class="fw-bolder fs-6 text-gray-800">
                <a href="{{ route('patients.show', ['patient' => $invoice->patient->id]) }}"
                   class="link-primary">{{ $invoice->patient->user->full_name }}</a></div>
        </div>
        <div class="mb-6">
        ...
        <div class="mb-6">
            ...
            <div
                class="fw-bolder text-gray-800 fs-6">{{ $invoice->patient->user->gender == 1 ? __('messages.user.female'): __('messages.user.male') }}</div>
        </div>
    </div>
</div>
