<div>
        ...
        <div class="card-body pt-9 pb-0">
                    ...
                    <div class="d-flex justify-content-between align-items-start flex-wrap mb-2">
                        <div class="d-flex flex-column">
                            <div class="d-flex align-items-center mb-2">
                                <a href="#"
                                   class="text-gray-800 text-hover-primary fs-2 fw-bolder me-4">{{$caseHandler->user->full_name}}</a>
                                <span
                                    class="badge badge-light-{{ $caseHandler->user->status === 1 ? 'success' : 'danger' }} fw-bolder ms-2 fs-8 py-1 px-3">{{ ($caseHandler->user->status === 1) ? __('messages.common.active') : __('messages.common.de_active') }}</span>
                            </div>
            ...
            <div class="d-flex overflow-auto h-55px">
                <ul class="nav nav-stretch nav-line-tabs nav-line-tabs-2x border-transparent fs-5 fw-bolder flex-nowrap">
                    <li class="nav-item">
                        <a class="nav-link text-active-primary me-6 active" data-bs-toggle="tab" href="#loverview">Overview</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link text-active-primary me-6" data-bs-toggle="tab" href="#lpayrolls">{{__('messages.my_payrolls')}}</a>
                    </li>
                </ul>
            </div>
        </div>
    ...
    <div class="tab-content" id="myTabContent">
        <div class="tab-pane fade show active" id="loverview" role="tabpanel">
            <div class="card mb-5 mb-xl-10">
                ...
                <div>
                    <div class="card-body  border-top p-9">
                        <div class="row mb-7">
                            ...
                            <div class="col-lg-8">
                                <span class="fw-bolder fs-6 text-gray-800">{{!empty($caseHandler->user->phone)?$caseHandler->user->phone:'N/A'}}</span>
                            </div>
                        </div>
                        <div class="row mb-7">
                            <label class="col-lg-4 fw-bold text-muted">{{ __('messages.user.gender')  }}</label>
                            <div class="col-lg-8">
                                <span class="fw-bolder fs-6 text-gray-800">{{ $caseHandler->user->gender == 0 ? 'Male' : 'Female' }}</span>
                            </div>
                        </div>
                        <div class="row mb-7">
                            <label class="col-lg-4 fw-bold text-muted">{{ __('messages.user.blood_group')  }}</label>
                            <div class="col-lg-8">
                                <span class="fs-6 badge badge-light-{{!empty($caseHandler->user->blood_group) ? 'success' : 'danger'}}">{{ !empty($caseHandler->user->blood_group) ? $caseHandler->user->blood_group : __('messages.common.n/a') }}</span>
                            </div>
                        </div>
                        <div class="row mb-7">
                            <label class="col-lg-4 fw-bold text-muted">{{ __('messages.user.dob')  }}</label>
                            <div class="col-lg-8">
                                <span class="fw-bolder fs-6 text-gray-800">{{ !empty($caseHandler->user->dob) ? \Carbon\Carbon::parse($caseHandler->user->dob)->format('jS M, Y') : __('messages.common.n/a') }}</span>
                            </div>
                        </div>
                        <div class="row mb-7">
                            ...
                            <div class="col-lg-8">
                                <span
                                    class="fw-bolder fs-6 text-gray-800">{{ !empty($caseHandler->user->designation) ? $caseHandler->user->designation : __('messages.common.n/a') }}</span>
                            </div>
                        </div>
                        <div class="row mb-7">
                            ...
                            <div class="col-lg-8">
                                <span
                                    class="fw-bolder fs-6 text-gray-800">{{ !empty($caseHandler->user->qualification) ? $caseHandler->user->qualification : __('messages.common.n/a')}}</span>
                            </div>
                        </div>
                        <div class="row mb-7">
                            ...
                            <div class="col-lg-8">
                                <span
                                    class="fw-bolder fs-6 text-gray-800 me-2">{{ !empty($caseHandler->user->created_at) ? $caseHandler->user->created_at->diffForHumans() : __('messages.common.n/a') }}</span>
                            </div>
                        </div>
                        <div class="row mb-7">
                            ...
                            <div class="col-lg-8">
                                <span
                                    class="fw-bolder fs-6 text-gray-800 me-2">{{ !empty($caseHandler->user->updated_at) ? $caseHandler->user->updated_at->diffForHumans() : __('messages.common.n/a') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="tab-pane fade" id="lpayrolls" role="tabpanel">
            <div class="card mb-5 mb-xl-10">
                <div class="card-header border-0">
                    <div class="card-title m-0">
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="table-responsive viewList">
                                    <table id="patientCases" class="display table table-responsive-sm table-striped align-middle table-row-dashed fs-6 gy-5 gx-5 dataTable no-footer w-100">
                                        <thead>
                                        ...
                                        @forelse($payrolls as $payroll)
                                            <tr>
                                                <td class="text-center">
                                                    <a href="{{url('employee-payrolls', $payroll->id)}}"><span class="badge badge-light-info fs-6">{{ $payroll->payroll_id }}</span></a>
                                                </td>
                                                <td>{{ $payroll->month }}</td>
                                                <td>{{ $payroll->year }}</td>
                                                <td class="text-right">
                                                ...
                                                <td class="text-center">
                                                    <span class="badge fs-6 badge-light-{{($payroll->status == 1) ? 'success' : 'danger'}}">{{ ($payroll->status) ? __('messages.employee_payroll.paid') : __('messages.employee_payroll.not_paid') }}</span>
                                                </td>
                                            </tr>
                                        @empty
                                            <tr>
                                                <td class="text-center" colspan="8">{{__('No data available in table')}}</td>
                                            </tr>
                                        @endforelse
                                        </tbody>
                                    </table>
