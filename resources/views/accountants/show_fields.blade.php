        ...
        <div class="card-body pt-9 pb-0">
                    ...
                    <div class="d-flex justify-content-between align-items-start flex-wrap mb-2">
                        <div class="d-flex flex-column">
                            <div class="d-flex align-items-center mb-2">
                                <a href="#"
                                   class="text-gray-800 text-hover-primary fs-2 fw-bolder me-4">{{$accountant->user->full_name}}</a>
                                <span
                                    class="badge badge-light-{{ $accountant->user->status === 1 ? 'success' : 'danger' }} fw-bolder ms-2 py-1 px-3">{{ ($accountant->user->status === 1) ? __('messages.common.active') : __('messages.common.de_active') }}</span>
                            </div>
            ...
            <div class="d-flex overflow-auto h-55px">
                <ul class="nav nav-stretch nav-line-tabs nav-line-tabs-2x border-transparent fs-5 fw-bolder flex-nowrap">
                    <li class="nav-item">
                        <a class="nav-link text-active-primary me-6 active" data-bs-toggle="tab" href="#aoverview">Overview</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link text-active-primary me-6" data-bs-toggle="tab" href="#apayrolls">{{__('messages.my_payrolls')}}</a>
                    </li>
                </ul>
            </div>
        </div>
                ...
                <div>
                    <div class="card-body  border-top p-9">
                        <div class="row mb-7">
                            ...
                            <div class="col-lg-8">
                                <span class="fw-bolder fs-6 text-gray-800">{{!empty($accountant->user->phone)?$accountant->user->phone:'N/A'}}</span>
                            </div>
                        </div>
                        <div class="row mb-7">
                            <label class="col-lg-4 fw-bold text-muted">{{ __('messages.user.gender')  }}</label>
                            <div class="col-lg-8">
                                <span class="fw-bolder fs-6 text-gray-800">{{ $accountant->user->gender == 0 ? 'Male' : 'Female' }}</span>
                            </div>
                        </div>
                        <div class="row mb-7">
                            <label class="col-lg-4 fw-bold text-muted">{{ __('messages.user.blood_group')  }}</label>
                            <div class="col-lg-8">
                                <span class="fs-6 badge badge-light-{{!empty($accountant->user->blood_group) ? 'success' : 'danger'}}">{{ !empty($accountant->user->blood_group) ? $accountant->user->blood_group : __('messages.common.n/a') }}</span>
                            </div>
                        </div>
                        <div class="row mb-7">
                            <label class="col-lg-4 fw-bold text-muted">{{ __('messages.user.dob')  }}</label>
                            <div class="col-lg-8">
                                <span class="fw-bolder fs-6 text-gray-800">{{ !empty($accountant->user->dob) ? \Carbon\Carbon::parse($accountant->user->dob)->format('jS M, Y') : __('messages.common.n/a') }}</span>
                            </div>
                        </div>
                        <div class="row mb-7">
                            ...
                            <div class="col-lg-8">
                                <span
                                    class="fw-bolder fs-6 text-gray-800">{{ !empty($accountant->user->designation) ? $accountant->user->designation : __('messages.common.n/a') }}</span>
                            </div>
                        </div>
                        <div class="row mb-7">
                            ...
                            <div class="col-lg-8">
                                <span
                                    class="fw-bolder fs-6 text-gray-800">{{ !empty($accountant->user->qualification) ? $accountant->user->qualification : __('messages.common.n/a')}}</span>
                            </div>
                        </div>
                        <div class="row mb-7">
                            ...
                            <div class="col-lg-8">
                                <span
                                    class="fw-bolder fs-6 text-gray-800 me-2">{{ !empty($accountant->user->created_at) ? $accountant->user->created_at->diffForHumans() : __('messages.common.n/a') }}</span>
                            </div>
                        </div>
                        <div class="row mb-7">
                            ...
                            <div class="col-lg-8">
                                <span
                                    class="fw-bolder fs-6 text-gray-800 me-2">{{ !empty($accountant->user->updated_at) ? $accountant->user->updated_at->diffForHumans() : __('messages.common.n/a') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="table-responsive viewList">
                                    ...
                                    <table id="accountantPayrolls" class="display table table-responsive-sm table-striped align-middle table-row-dashed fs-6 gy-5 gx-5 dataTable no-footer w-100">
                                        <thead>
                                        ...
                                        @foreach($payrolls as $payroll)
                                            <tr>
                                                ...
                                                <td class="text-center"><span class="badge badge-light-{{($payroll->status == 1  ? 'success' : 'danger')}}">{{ ($payroll->status) ? __('messages.employee_payroll.paid') : __('messages.employee_payroll.not_paid') }}</span></td>
                                            </tr>
                                        @endforeach
                                        </tbody>
                                    </table>
